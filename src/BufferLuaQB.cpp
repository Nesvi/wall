...
int BufferLuaQB_setEditable(lua_State* L){
	BufferLuaQB::getInstance(L)->setEditable(lua_toboolean(L,2));
	return 0;
}
...
void BufferLuaQB::load(lua_State* L){
	...
	lua_register(L,"QBBuffer_setEditable",BufferLuaQB_setEditable);
